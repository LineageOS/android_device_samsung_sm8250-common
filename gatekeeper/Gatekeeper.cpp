...
Gatekeeper::Gatekeeper() {
  int ret =
      hw_get_module_by_class(GATEKEEPER_HARDWARE_MODULE_ID, NULL, &module);
  device = NULL;
  ...
  if (ret < 0) {
    LOG_ALWAYS_FATAL_IF(ret < 0, "Unable to open GateKeeper MDFPP HAL");
  }
}
...
Gatekeeper::~Gatekeeper() {
  if (device != nullptr) {
    ...
    if (ret < 0) {
      ALOGE("Unable to close GateKeeper MDFPP HAL");
    }
  }
  dlclose(module->dso);
}
...
Return<void> Gatekeeper::enroll(uint32_t uid,
  ...
  int ret = device->enroll(device, uid, currentPasswordHandle.data(),
                           ...
                           currentPassword.size(), desiredPassword.data(),
                           desiredPassword.size(), &enrolled_password_handle,
                           &enrolled_password_handle_length);
  if (!ret) {
    rsp.data.setToExternal(enrolled_password_handle,
                           enrolled_password_handle_length, true);
...
Return<void> Gatekeeper::verify(uint32_t uid, uint64_t challenge,
  ...
  int ret =
      device->verify(device, uid, challenge, enrolledPasswordHandle.data(),
...
// bool Gatekeeper::isMdfppGatekeeper()
bool Gatekeeper::isMdfppInstance() {
  if (device != nullptr && device->common.module->name != nullptr) {
    const char *name = device->common.module->name;
    const char *target = "MDFPP Gatekeeper";
    size_t len = strlen(target);

    if (memcmp(target, name, len) == 0) {
      ALOGV("MDFPP Gatekeeper loaded");
      return 1;
    }
  }
  return 0;
...
void Gatekeeper::syncSecnvm() {
  int fd = open("/mnt/vendor/persist/secnvm", O_RDONLY);

  if (fd >= 0) {
    int ret = fsync(fd);
    if (ret < 0) {
      ALOGE("syncSecnvm: fsync failed : fd:%d", fd);
    }
    close(fd);
    ALOGV("syncSecnvm: success");
    return;
  }

  ALOGE("syncSecnvm: could not open : %s: %s", "/mnt/vendor/persist/secnvm",
        strerror(errno));
  return;
}
